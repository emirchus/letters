...
export function ChordDiagram({chordName}: ChordDiagramProps) {
  ...
  const barreChord = useMemo(
    () => chord.frets.filter((fret) => fret !== null && fret !== 0).length > 4,
    [chord],
  );
  const barrePosition = useMemo(
    () =>
      barreChord
        ? Math.min(...frets.filter((fret): fret is number => fret !== null && fret !== 0))
        : null,
    [barreChord, frets],
  );
